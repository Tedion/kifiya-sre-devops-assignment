# Postmortem Template

## Incident Summary

**Incident ID**: INC-YYYY-MMDD-001  
**Date**: YYYY-MM-DD  
**Duration**: HH:MM (start time - end time)  
**Severity**: P0 / P1 / P2 / P3  
**Status**: Resolved / Mitigated  

### Executive Summary
[2-3 sentence summary of what happened, impact, and resolution]

### Impact
- **Services Affected**: [List services]
- **Users Affected**: [Number/Percentage]
- **Revenue Impact**: [If applicable]
- **Duration**: [Time]
- **Geographic Impact**: [Regions affected]

---

## Timeline

### Detection
- **Time**: HH:MM UTC
- **Method**: [Alert/User Report/Monitoring]
- **Initial Assessment**: [What was observed]

### Response
- **Time**: HH:MM UTC
- **Incident Commander**: [Name]
- **Team Assembled**: [Names/Roles]
- **Communication**: [Channels used]

### Investigation
- **Time**: HH:MM UTC
- **Actions Taken**: [List key actions]
- **Findings**: [What was discovered]

### Resolution
- **Time**: HH:MM UTC
- **Fix Applied**: [Description]
- **Verification**: [How fix was verified]

### Post-Incident
- **Time**: HH:MM UTC
- **Monitoring**: [What was monitored]
- **Follow-up Actions**: [Immediate actions]

---

## Root Cause Analysis

### What Happened
[Detailed description of the incident]

### Why It Happened
[Root cause analysis - use "5 Whys" if helpful]

1. **Immediate Cause**: [Direct trigger]
2. **Contributing Factors**: [List factors that contributed]
3. **Root Cause**: [Underlying systemic issue]

### Detection Gap
[Why wasn't this detected earlier?]
- Monitoring gaps
- Alert thresholds
- Logging deficiencies

---

## Impact Analysis

### Technical Impact
- Services unavailable
- Data loss/corruption
- Performance degradation
- Security implications

### Business Impact
- User experience
- Revenue loss
- Reputation damage
- SLA violations

### Customer Impact
- Support tickets created
- Customer communications sent
- Refunds/credits issued

---

## Resolution

### Immediate Actions Taken
1. [Action 1]
2. [Action 2]
3. [Action 3]

### Long-term Fix
[Description of permanent solution]

### Verification
[How we verified the fix works]

---

## Lessons Learned

### What Went Well
- [Positive aspect 1]
- [Positive aspect 2]
- [Positive aspect 3]

### What Went Wrong
- [Issue 1]
- [Issue 2]
- [Issue 3]

### What Could Be Improved
- [Improvement 1]
- [Improvement 2]
- [Improvement 3]

---

## Action Items

| ID | Action Item | Owner | Priority | Due Date | Status |
|----|-------------|-------|----------|----------|--------|
| AI-001 | [Description] | [Name] | High/Medium/Low | YYYY-MM-DD | Open/In Progress/Done |
| AI-002 | [Description] | [Name] | High/Medium/Low | YYYY-MM-DD | Open/In Progress/Done |
| AI-003 | [Description] | [Name] | High/Medium/Low | YYYY-MM-DD | Open/In Progress/Done |

### High Priority Actions
[Detailed description of critical action items]

### Medium Priority Actions
[Description of important but not urgent items]

### Low Priority Actions
[Description of nice-to-have improvements]

---

## Metrics

### Time Metrics
- **MTTD (Mean Time To Detect)**: [Time]
- **MTTA (Mean Time To Acknowledge)**: [Time]
- **MTTR (Mean Time To Resolve)**: [Time]
- **MTBF (Mean Time Between Failures)**: [If applicable]

### Impact Metrics
- **Error Rate Peak**: [Percentage]
- **Requests Affected**: [Number]
- **Revenue Impact**: [Amount]
- **Customer Complaints**: [Number]

---

## Follow-up

### Review Date
[Date for follow-up review]

### Stakeholders
- [Name/Role]
- [Name/Role]

### Communication
- [ ] Internal team notified
- [ ] Management briefed
- [ ] Customer communication sent (if applicable)
- [ ] Status page updated
- [ ] Postmortem shared with stakeholders

---

## Appendix

### Logs and Evidence
- [Links to relevant logs]
- [Screenshots]
- [Metrics graphs]

### Related Incidents
- [Link to similar past incidents]

### References
- [Runbooks used]
- [Documentation consulted]
- [External resources]

